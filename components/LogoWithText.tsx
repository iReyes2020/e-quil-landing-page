...
const LogoWithText = ({ size = "md" }: { size?: "sm" | "md" | "lg" }) => {
  const sizeMap = {
    sm: { logoWidth: 40, logoHeight: 40, fontSize: 16, taglineSize: 10 },
    md: { logoWidth: 60, logoHeight: 60, fontSize: 24, taglineSize: 12 },
    lg: { logoWidth: 120, logoHeight: 120, fontSize: 48, taglineSize: 16 },
  };
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <div className="text-center">
        <div
          className="font-extrabold text-brand-white tracking-wider"
          style={{ fontSize: `${config.fontSize}px` }}
        >
          E-QUIL
        </div>
        <div
          className="text-brand-white/70 tracking-widest font-semibold"
          style={{ fontSize: `${config.taglineSize}px` }}
        >
          BEAT BURNOUT.
        </div>
      </div>
